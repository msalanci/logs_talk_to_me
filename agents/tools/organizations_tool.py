...
"""
organizations_tool.py — Strands @tool for querying AWS Organizations structure.

Registered on the organizations_agent as @tool query_organizations_api.

Calls AWS Organizations boto3 APIs directly (DescribeOrganization, ListAccounts, ListPolicies, ListOrganizationalUnitsForParent, etc.). 
No Athena/Glue/S3 infrastructure needed — pure API-based tool.

Management-account only — no cross-account STS role assumption needed.
Organizations API is global (no region parameter required); the `region` parameter on the tool exists for interface consistency only.
"""
